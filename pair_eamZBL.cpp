...
PairEAMZBL::PairEAMZBL(LAMMPS *lmp) : Pair(lmp)
{
  restartinfo = 0;
  manybody_flag = 1;
  embedstep = -1;
...
void PairEAMZBL::compute(int eflag, int vflag)
{
  ...
  // ------------------- Start ZBL repulsive portion -------------------
  double global_a_0 = 0.529;   //Bohr radius
  double global_epsilon_0 = 0.00552635;   //permittivity of vacuum
  double global_e = 1.0;  //electron charge
  double esq = pow(global_e,2.0);  //e_squared
  double Z_i = -99;
  double Z_j = -99;
  double a_ij;
  double premult;
  ...
  // ------------------- START ZBL repulsive portion ------------------
      //assign atomic_number to Ni Co Cr, respectively
      Z_i = -99;
      Z_j = -99;    
      if (itype==1) {
        Z_i=28;
      ...
      // parameter a in the screening function
      a_ij = (0.8854*global_a_0) /
       (pow(Z_i,0.23) + pow(Z_j,0.23));  
      // prefator for the coulomb repulsive term
      premult = (Z_i * Z_j * esq)/(4.0*PI*global_epsilon_0);
      // variable of the screening function
      r_ov_a = r/a_ij;
      // ZBL screening function
      phi_zbl = 0.1818*exp(-3.2*r_ov_a) + 0.5099*exp(-0.9423*r_ov_a) +
       0.2802*exp(-0.4029*r_ov_a) + 0.02817*exp(-0.2016*r_ov_a);
      // ZBL portion (Coulomb + ZBL screening function)
      eng_ZBL = premult*(1.0/r)*phi_zbl;
      // derivative of phi_zbl
      dphi_zbl = (1.0/a_ij) * (-3.2*0.1818*exp(-3.2*r_ov_a) -
                              0.9423*0.5099*exp(-0.9423*r_ov_a) -
                              0.4029*0.2802*exp(-0.4029*r_ov_a) -
      			      0.2016*0.02817*exp(-0.2016*r_ov_a));
      // derivative of the ZBL portion (not really force) 
      fforce_ZBL = premult*(-1.0/rsq)*phi_zbl + premult*dphi_zbl/r; 
  // ------------------- END ZBL repulsive portion ----------------------
        ...
        // rewrite the total force, only the two-body terms need to be modified
	psip= fp[i]*rhojp + fp[j]*rhoip + 
	      (-F_fermi_d(r) * eng_ZBL + (1.0 - F_fermi(r))*fforce_ZBL +
              F_fermi_d(r)* phi + F_fermi(r)*phip);
        ...
        // rewrite the line by replacing phi with ZBL terms
	if (eflag) evdwl = scale[itype][jtype]*((1.0-F_fermi(r))*eng_ZBL+F_fermi(r)*phi);
...
void PairEAMZBL::settings(int narg, char **/*arg*/)
{
...
void PairEAMZBL::coeff(int narg, char **arg)
{
...
double PairEAMZBL::init_one(int i, int j)
{
...
void PairEAMZBL::read_file(char *filename)
{
...
void PairEAMZBL::interpolate(int n, double delta, double *f, double **spline)
{
...
void PairEAMZBL::grab(FILE *fptr, int n, double *list)
{
  char *ptr;
  char line[MAXLINE];
...
double PairEAMZBL::single(int i, int j, int itype, int jtype,
...
int PairEAMZBL::pack_forward_comm(int n, int *list, double *buf,
...
void PairEAMZBL::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int PairEAMZBL::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairEAMZBL::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
double PairEAMZBL::memory_usage()
{
...
void PairEAMZBL::swap_eam(double *fp_caller, double **fp_caller_hold)
{
  double *tmp = fp;
  fp = fp_caller;
  *fp_caller_hold = tmp;
...
void *PairEAMZBL::extract(const char *str, int &dim)
{
  dim = 2;
...
// the smoothing function needed for the ZBL implementation

double PairEAMZBL::F_fermi(double r)
{
  double ZBLexpscale=14;  //The parameter controls the sharpness of the smoothing
  double ZBLcut=1.25;  //usualy one half of the nerest neighbor distance
  return 1.0 / (1.0 + exp(-ZBLexpscale*(r-ZBLcut)));
 // return 1.0;
}

// smoothing function derivative with respect to r (ZBL implementation)

double PairEAMZBL::F_fermi_d(double r)
{
  double ZBLexpscale=14;
  double ZBLcut=1.25;
   return ZBLexpscale*exp(-ZBLexpscale*(r-ZBLcut)) /
    pow(1.0 + exp(-ZBLexpscale*(r-ZBLcut)),2.0);
  //  return 0.0;
}
